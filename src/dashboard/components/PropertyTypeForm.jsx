// components/forms/PropertyTypeForm.tsx
...
const validationSchema = Yup.object({
    nameEn: Yup.string()
        ...
        .matches(/^[a-zA-Z\s\-&,]+$/, 'English name can only contain letters, spaces, hyphens, commas, and ampersands'),
    nameAr: Yup.string()
        .required('Arabic name is required')
        .min(2, 'Arabic name must be at least 2 characters')
        .max(100, 'Arabic name must be less than 100 characters')
        .matches(/^[\u0600-\u06FF\s\-&،]+$/, 'Arabic name can only contain Arabic letters, spaces, hyphens, Arabic commas, and ampersands')
});
        ...
        <form onSubmit={formik.handleSubmit} className="space-y-6" noValidate>
            <div>
                <h3 className="text-lg font-semibold text-neutral-900   mb-4">
                    {item ? t('dashboard.propertyTypeForm.editType') : t('dashboard.propertyTypeForm.createType')}
                </h3>
                <p className="text-sm text-neutral-600   mb-6">
                    {t('dashboard.propertyTypeForm.description')}
                </p>
            </div>
            ...
            <div className="space-y-4">
                <div>
                    <Input
                        ...
                        placeholder={t('dashboard.propertyTypeForm.nameEnPlaceholder')}
                ...
                <div>
                    <Input
                        ...
                        onChange={formik.handleChange}
                        onBlur={formik.handleBlur}
                        placeholder={t('dashboard.propertyTypeForm.nameArPlaceholder')}
            ...
            <div className="flex gap-3 justify-end pt-6 border-t border-neutral-400">
                ...
                <Button
                    ...
                    className="min-w-30"
                >
                    {item ? t('dashboard.propertyTypeForm.updateType') : t('dashboard.propertyTypeForm.createType')}
                </Button>
            </div>
        </form>
    );
