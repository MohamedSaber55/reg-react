...
import { fetchUnitModelById, clearCurrentUnitModel } from '@/store/slices/unitModelSlice';
...
export default function UnitModelDetailsClient({ projectId, stageId, unitId }) {
    ...
    const resolvedStageId = typeof params?.stageId === 'string' && params.stageId !== 'placeholder'
        ? params.stageId
        : stageId;
    const resolvedUnitId = typeof params?.unitId === 'string' && params.unitId !== 'placeholder'
        ? params.unitId
        : unitId;
    const currentLang = i18n.language;
    const isRTL = currentLang === 'ar';

    const { currentUnitModel, operationLoading } = useAppSelector((state) => state.unitModel);
    ...
    const getActiveImages = () => {
        if (!currentUnitModel?.images || currentUnitModel.images.length === 0) {
            return [{
                id: 'default-image',
        ...
        return currentUnitModel.images.length > 0 ? currentUnitModel.images : [{
            id: 'default-image',
            imageUrl: defaultPropertyImage.src,
            isMain: true
        }];
    ...
    const activeImages = currentUnitModel ? getActiveImages() : [];
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values, { resetForm, setSubmitting }) => {
            try {
                const unit = currentUnitModel;
                const formattedPrice = formatPriceFull(unit.startingPrice, {
                    t: t,
...
---
Unit Model Inquiry Details:
---
Unit Model: ${unit.name}
Model Code: ${unit.modelCode || 'N/A'}
Stage: ${unit.stageName || 'N/A'}
Starting Price: ${formattedPrice || 'N/A'}
Area: ${unit.area || 'N/A'} m²
Bedrooms: ${unit.bedrooms || 'N/A'}
Bathrooms: ${unit.bathrooms || 'N/A'}
Availability: ${unit.isAvailable ? 'Available' : 'Not Available'}
Unit Model ID: ${unit.id}
                `.trim();
    ...
    useEffect(() => {
        if (resolvedUnitId && resolvedUnitId !== 'placeholder') {
            dispatch(fetchUnitModelById(resolvedUnitId));
        }
        dispatch(fetchContactPhones());
        dispatch(fetchContactEmails());

        return () => {
            dispatch(clearCurrentUnitModel());
        };
    }, [dispatch, resolvedUnitId]);

    const displayPhones = contactPhones.slice(0, 2);
    const displayEmails = contactEmails.slice(0, 2);

    if (operationLoading || !currentUnitModel) {
        return (
    ...
    const displayPrice = formatPrice(unit.startingPrice, {
        t: t,
        currency: 'models.currency'
    });

    const pricePerSqm = unit.area && unit.startingPrice
        ? formatPrice(unit.startingPrice / unit.area, {
            t: t,
            currency: 'models.currency'
        })
        : null;
    ...
    const amenities = [
        { icon: FaBed, label: t('units.bedrooms'), value: unit.bedrooms, show: true },
        { icon: FaBath, label: t('units.bathrooms'), value: unit.bathrooms, show: true },
        { icon: FiMaximize, label: t('units.area'), value: `${unit.area} m²`, show: true },
    ];
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-2 space-y-8">
                                    ...
                                    <div className="relative w-full aspect-video rounded-2xl overflow-hidden bg-neutral-100 flex items-center justify-center">
                                        ...
                                        <div className="absolute inset-0 bg-black/20 flex flex-col items-center justify-center text-white p-6 text-center">
                                            ...
                                            <div className="text-sm opacity-90">
                                                {t('property.defaultImageDisplayed', 'Default image displayed')}
                                            </div>
                                        </div>
                                    </div>
                                ) : (
                        ...
                        >
                            <div className="flex items-start justify-between mb-4">
                                <div className="flex-1">
                                    <div className="flex items-center gap-3 mb-3">
                                        ...
                                        {unit.isAvailable ? (
                                            <span className="px-3 py-1 bg-success-100 text-success-700 text-sm font-semibold rounded-full">
                                                {t('units.available', 'Available')}
                                            </span>
                                        ) : (
                                            <span className="px-3 py-1 bg-error-100 text-error-700 text-sm font-semibold rounded-full">
                            ...
                            <div className="text-xs text-third-500 mt-1">
                                {t('units.startingFrom', 'Starting from')}
                            </div>
                        </motion.div>
                        ...
                        {unit.description && (
                            ...
                            >
                                <h2 className="text-2xl font-bold text-third-900 mb-4 font-serif flex items-center gap-2">
                                    <FiPackage className="text-primary-600" />
                                    {t('units.description', 'Description')}
                                </h2>
